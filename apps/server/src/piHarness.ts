...
export const PI_PROVIDER_SETUP_MESSAGE =
  "k3code embeds Pi through the Pi Node SDK. Authenticate Pi outside k3code through the Pi CLI (`pi` or `bunx pi`) and `/login`, or populate ~/.pi/agent/auth.json / provider env vars. k3code intentionally disables Pi packages, extensions, prompt templates, skills, themes, AGENTS, and custom system-prompt discovery so k3code remains the only source of workspace instructions here.";

export const PI_PLAN_MODE_PROMPT_PREFIX = `<collaboration_mode name="plan">
You are in k3code plan mode.
...
- When you present the finalized plan, wrap it in <proposed_plan>...</proposed_plan> so k3code can render it specially.
</collaboration_mode>`;
